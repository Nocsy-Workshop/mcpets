package fr.nocsy.mcpets.mythicmobs.placeholders;

import javax.annotation.Nullable;

import fr.nocsy.mcpets.data.Pet;

import io.lumine.mythic.core.utils.annotations.MythicPlaceholder;
import io.lumine.mythic.core.skills.placeholders.PlaceholderContext;
import io.lumine.mythic.core.skills.placeholders.types.GenericPlaceholderTypes.StringPlaceholder;

@MythicPlaceholder(placeholder = "pet.level.name", version = "5.9")
public class PetLevelNamePlaceholder extends PetPlaceholder<String> implements StringPlaceholder {

    public PetLevelNamePlaceholder(GenericPlaceholderArguments context) {
        super(context);
    }

    @Nullable
    @Override
    public String applyWithMetaKeywords(PlaceholderContext context) {
        Pet pet = getPet(context);

        if (pet == null || pet.getPetStats() == null) {
            return "null";
        }

        return pet.getPetStats().getCurrentLevel().getLevelName();
    }

}
